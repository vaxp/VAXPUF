...
static void draw_control_center_content(VaxpCanvas* canvas, int width, int height) {
    ...
    VaxpColor vol_color = g_state.audio.muted 
        ? vaxp_color_rgba(100, 100, 100, 255) 
        : vaxp_color_rgba(100, 200, 150, 255);
    VaxpPaint vol_fill = vaxp_paint_fill(vol_color);
    vaxp_canvas_draw_rounded_rect(canvas, (VaxpRectF){ 15, vol_y, (width - 30) * vol_percent, 35 }, 10.0f, &vol_fill);
    
    /* Speaker icon */
    VaxpPaint speaker_p = vaxp_paint_fill(vaxp_color_rgba(255, 255, 255, 255));
    /* Speaker body (small rectangle) */
    vaxp_canvas_draw_rect(canvas, (VaxpRectF){ 26, vol_y + 13, 6, 9 }, &speaker_p);
    /* Speaker cone (larger part) */
    vaxp_canvas_draw_rect(canvas, (VaxpRectF){ 32, vol_y + 10, 4, 15 }, &speaker_p);
    /* Sound waves (if not muted) */
    if (!g_state.audio.muted && g_state.audio.volume > 0) {
        VaxpPaint wave = vaxp_paint_stroke(vaxp_color_rgba(255, 255, 255, 180), 1.5f);
        vaxp_canvas_draw_circle(canvas, 40, vol_y + 17, 5, &wave);
        if (g_state.audio.volume > 50) {
            vaxp_canvas_draw_circle(canvas, 40, vol_y + 17, 9, &wave);
        }
    }
    /* Mute line (if muted) */
    if (g_state.audio.muted) {
        VaxpPaint mute = vaxp_paint_stroke(vaxp_color_rgba(255, 80, 80, 255), 2.0f);
        vaxp_canvas_draw_line(canvas, 25, vol_y + 8, 45, vol_y + 27, &mute);
    }
    
    /* Volume percentage text */
    char vol_str[8];
    snprintf(vol_str, sizeof(vol_str), "%d%%", g_state.audio.volume);
    VaxpPaint vol_text = vaxp_paint_fill(vaxp_color_rgba(255, 255, 255, 200));
    vaxp_canvas_draw_text(canvas, vol_str, width - 60, vol_y + 24, NULL, &vol_text);
...
int main(int argc, char** argv) {
    ...
    g_state.audio_connected = vaxp_audio_init();
    if (g_state.audio_connected) {
        vaxp_audio_get_state(&g_state.audio);
                ...
                case ButtonPress:
                    if (event.xbutton.window == panel_window) {
                        ...
                        if (mx >= 15 && mx <= cc_width - 15 && my >= 175 && my <= 210) {
                            ...
                            if (g_state.audio_connected) {
                                vaxp_audio_set_volume(new_volume);
