...
void dbus_emit_volume_changed(gint volume) {
    if (!dbus_conn) return;
    g_dbus_connection_emit_signal(dbus_conn, NULL, "/org/vaxp/Audio",
        "org.vaxp.Audio", "VolumeChanged", g_variant_new("(i)", volume), NULL);
}

void dbus_emit_mute_changed(gboolean muted) {
    if (!dbus_conn) return;
    g_dbus_connection_emit_signal(dbus_conn, NULL, "/org/vaxp/Audio",
        "org.vaxp.Audio", "MuteChanged", g_variant_new("(b)", muted), NULL);
}

void dbus_emit_devices_changed(void) {
    if (!dbus_conn) return;
    g_dbus_connection_emit_signal(dbus_conn, NULL, "/org/vaxp/Audio",
        "org.vaxp.Audio", "DevicesChanged", NULL, NULL);
}

void dbus_emit_apps_changed(void) {
    if (!dbus_conn) return;
    g_dbus_connection_emit_signal(dbus_conn, NULL, "/org/vaxp/Audio",
        "org.vaxp.Audio", "AppsChanged", NULL, NULL);
}
