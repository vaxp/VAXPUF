 ...
 * Implementation - connects to org.venom.Audio D-Bus service
 */
...
#define AUDIO_BUS_NAME    "org.venom.Audio"
#define AUDIO_OBJECT_PATH "/org/venom/Audio"
#define AUDIO_INTERFACE   "org.venom.Audio"
...
static GVariant* call_method(const char* method, GVariant* params) {
    if (!audio_proxy) {
        if (!venom_audio_init()) return NULL;
    }
...
bool venom_audio_toggle_mute(void) {
    bool current = venom_audio_get_muted();
    return venom_audio_set_muted(!current);
}
...
bool venom_audio_volume_up(int step) {
    int current = venom_audio_get_volume();
    return venom_audio_set_volume(current + step);
}
...
bool venom_audio_get_state(VenomAudioState* state) {
    ...
    state->volume = venom_audio_get_volume();
    state->muted = venom_audio_get_muted();
